                ...
                switch ($this->client){
                    ...
                    case 2://导游端获取用户信息
                        $info = $Model
                            ...
                            ->field('ui.realname,oi.phone,ui.nickname,ui.phone as mp,oi.start_addr as server_addr,oi.server_start_time,ui.head_image,ui.lon,ui.lat,oi.start_addr as server_addr,oi.guide_type')
                            ->find();
 ...
 */
    ...
    private function get_order_info($order_number) {
        ...
        if ($this->client == 2) {
            $info = $Model
                ->field('oi.realname,oi.phone,ui.nickname,ui.phone as mp,ui.head_image,o.order_number,o.server_status,o.pay_status,o.order_time,o.cancel_time,o.order_money,o.rebate_money,o.tip_money,o.pay_money,o.pay_type,oi.server_start_time,oi.server_end_time,oi.server_price,oi.guide_type,oi.charging_type,oi.end_addr,c.content,c.star,c.tag')
                ->join('left join __ORDER_INFO__ as oi on o.order_number=oi.order_number')
                ->join('left join __COMMENT__ as c on c.order_number=o.order_number')
                ->join('left join __USER_MEMBER_INFO__ as ui on ui.uid=o.uuid')
                ->where(array('o.order_number' => $order_number))
    ...
    public function order_status($server_status, $pay_status) {
        ...
        if ($server_status == 2 && $pay_status == 2)//已关闭（已取消并支付违约金）
            $order_status = 8;
        if ($server_status == 7 && $pay_status == 0)//已接单，未开始
            $order_status = 1;
        return $order_status;
    }
}
