...
class IndexController extends ApiController
{
    ...
    public function start_server()
    {
        switch ($this->_method) {
            case "post":
                ...
                $Model_order = M('order');
                //检测订单是被已经被取消
                if (!$Model_order->where(array('order_number' => $order_number, 'server_status' => 1))->count())
                ...
                if (false !== $Model->execute('alter table __ORDER__ ENGINE=innodb') && FALSE !== $Model->execute('alter table __ORDER_INFO__ ENGINE=innodb')) {
                    ...
                    if (!$Model_order_info->where(array_merge($map, array('server_type' => 0)))->save($data_order_info) || !$Model_order->where(array_merge($map, array('server_status' => 1, 'pay_status' => 0)))->save($data_order)) {
            ...
            case "post":
                ...
                if ($order_info) {
                    ...
                    if (false !== $Model->execute('alter table __ORDER__ ENGINE=innodb') && FALSE !== $Model->execute('alter table __ORDER_INFO__ ENGINE=innodb')) {
                        ...
                        //订单金额计算
                        $now_time = NOW_TIME;
                        $server_seconds = bcsub($now_time, $order_info['server_start_time'], 0); //服务时间（秒）
                        $tmp = explode(':', gmstrftime('%H:%M:%S', $server_seconds));
                        ...
                        $order_money = bcmul($server_time_long, $order_info['server_price'], 2);
    ...
    public function version_update(){
        switch ($this->_method){
            case 'get':
                $client = I('get.client',1);
                if(empty($client) || !in_array($client,array('1','2')))
                    $this->client_return(0,'检查版本更新错误');
                if($result = M('version')->field('version_code,version_title,update_content,version_url')->order('version_code desc')->where('client=' . $client)->find())
                    $this->client_return(1, '获取成功！', $result);
                $this->client_return(0, '获取失败！');
            break;
        }
