...
class UserController extends ApiController {
    ...
    public function register() {
        ...
        $User = new UserApi;
        $uid = $User->register($mobile, $password, $this->client);
        if ($uid > 0)
    ...
    public function register_guide_info() {
        switch ($this->_method) {
            ...
            case 'post':
                ...
                $User = new UserApi;
                if (0 < $uid = $User->register_info($uid))
        ...
        switch ($this->_method) {
            ...
            case 'post':
                ...
                if (0 < $uid) { //UC登录成功
                    ...
                    $user_info['idcard'] = Aes_encrypt_decrypt($user_info['idcard'],2);

                    /*
                     * 账户资料审核失败
                     */
                    if($user_info['is_auth'] == -1){
                        $tourist = M('tourist_area')->where(array('id'=>$user_info['tourist_id']))->field('tourist_code,area_code')->find();
                        $image_info = M('GuideMemberInfo')->where(array('uid'=>$uid))->field('idcard_front,idcard_back,guide_idcard')->find();
                        $data = array(
                            'uid'   =>  $uid,
                            'realname'  =>  $user_info['realname'],
                            'idcard'  =>  $user_info['idcard'],
                            'guide_type'  =>  $user_info['guide_type'],
                            'email'  =>  $user_info['email'],
                            'phone'  =>  $user_info['phone'],
                            'tourist_code'  =>  $tourist['tourist_code'],
                            'area_code'  =>  $tourist['area_code'],
                            'head_image'    =>  $user_info['head_image'],
                            'idcard_front'    =>  get_pic_url().$image_info['idcard_front'],
                            'idcard_back'    =>  get_pic_url().$image_info['idcard_back'],
                            'guide_idcard'    =>  get_pic_url().$image_info['guide_idcard'],
                        );
                        $this->client_return(5,'账户信息审核失败！',$data);
                    }
     ...
     * 上传push推送id
     */
    public function update_push_id(){
        switch ($this->_method){
            case 'post':
                $uid = I('post.uid');
                $push_id = I('post.push_id');
                ...
                if ($push_id === 'null')
                    $this->client_return(2,'推送id不能为空！');
                switch ($this->client){
                    case 1: $Model = M('user_member_info');   break;
                    case 2: $Model = M('guide_member_info');   break;
                }
                if(false !== $Model->where(array('uid'=>$uid))->setField('push_id',$push_id))
                    $this->client_return(1,'推送id上传成功！');
                $this->client_return(0,'推送id上传失败！');
                break;
        }
    }
    ...
    public function check_idcard(){
        switch ($this->_method){
            case 'get':
                ...
                if(Aes_encrypt_decrypt($tmp,2) == $idcard)
    ...
    public function invite_info(){
        switch ($this->_method){
            case 'get':
                $now_page = I('now_page',1);
                $page_number = I('page_number',15);
                if($this->client != 2)
                    $this->client_return(0,'非法操作！');
                $uid = I('get.uid');
                if(empty($uid))
                    $this->client_return(0,'用户id不能为空！');
                $user = M('guide_member_info')->where(array('uid'=>$uid))->field('invite_code,guide_type')->find();
                if(!$user)
                    $this->client_return(0,'参数错误！');
                //获取用户直属下级用户
                $info['count']  =   M('invite')->where(array('invite_id'=>$uid))->count();
                $info['invite'] =   $user['invite_code'];
                if($all = M('invite')->where(array('invite_id'=>$uid))->field('invited_id')->select()){
                    $temp = array();
                    foreach ($all as $v){
                        $map = array(
                            'uid'=>$uid,
                            'child_id'=> $v['invited_id'],
                            'deal_type' =>  1,
                        );
                        array_push($temp,M('amount')->where($map)->sum('deal_amount'));
                    }
                }
                $info['total_amount'] = sprintf("%.2f", array_sum($temp));
                $invite_info = M('invite')->where(array('invite_id'=>$uid))->field('invited_id')->page($now_page,$page_number)->select();
                if($invite_info){
                    foreach ($invite_info as $val){
                        $map = array(
                            'uid'=>$uid,
                            'child_id'=>$val['invited_id'],
                            'deal_type' =>  1,
                        );
                        $amount = M('amount')->where($map)->sum('deal_amount');
                        $info['info'][] = array(
                            'amount'    =>  $amount ? $amount : -1,
                            'mobile'    =>  M('guide_member as gm')->getFieldById($val['invited_id'],'mobile'),
                        );
                    }
                }
    ...
    private function showRegError($code = 0) {
        switch ($code) {
            ...
            case -21: $error = '所属景区和景区码为空或不正确！';
                break;
            case -22: $error = '注册服务类型不符，不能使用邀请码！';
                break;
            default: $error = '未知错误！';
        }
        return $error;
        ...
        switch($this->_method){
            case 'post':
                ...
                if(isset($idcard) && !empty($idcard))
                    $data['idcard'] = Aes_encrypt_decrypt($idcard,1);
                $Model = M('user_linkman');
                if(false !== $Model->where(array('id'=>$id))->save($data))
        ...
        switch($this->_method){
            case 'get':
                ...
                if($list = $Model->where(array('uuid'=>$uid))->field('id,realname,phone,idcard')->select()){
                    foreach ($list as &$val) {
                        $val['idcard'] = Aes_encrypt_decrypt($val['idcard'],2);
                    }
                }
    ...
    public function del_bankcard(){
        switch ($this->_method){
            case 'post':
                $bankcard_id = I('post.bankcard_id',0);
                if(empty($bankcard_id) || !is_numeric($bankcard_id))
                    $this->client_return(0,'参数不能为空！');
                $Model = M('guide_bank_info');
                if(!$Model->where(array('id'=>$bankcard_id))->delete())
                    $this->client_return(0,'删除失败！');
                $this->client_return(1,'删除成功！');
                break;
        }
    ...
    public function create_withdraw(){
        switch($this->_method){
            case 'post':
                ...
                $action_amount = M('guide_member_info')->getFieldByUid($uid,'action_amount');
                ...
                if($bankcard_info = $Model_guide_bank_info->where(array('uid'=>$uid,'id'=>$bankcard_id))->field('bankcard_number,card_username,bank_number')->find()){
                    ...
                    if (FALSE !== $Model->execute('alter table __GUIDE_MEMBER_INFO__ ENGINE=InnoDB') && FALSE !== $Model->execute('alter table __GUIDE_WITHDRAW__ ENGINE=InnoDB')) {
                        $Model_withdraw->startTrans();
                        if(!$Model_withdraw->add($data_withdraw) || false == $Model_guide_info->where(array('uid'=>$uid))->save($data_guide)){
    ...
    public function amount_info(){
        switch($this->_method){
            case 'get':
                ...
                $Model_guide_info = M('guide_member_info');
                if($result = $Model_guide_info->where(array('uid'=>$uid))->field('now_total_amount as total_amount,action_amount as withdraw_amount')->find()){
    ...
    public function bill_list(){
        switch ($this->_method){
            case 'get':
                ...
                $now_page = I('get.now_page',1);
                $page_number = I('get.page_number',10);
                $screen = I('get.screen',0,'intval');//数据获取类型：0-获取全部；1-获取服务明细；2-获取返利明细；3-获取提现明细
                if(empty($uid) || !is_numeric($uid))
                    $this->client_return(0,'用户id不正确！');
                $this->withdraw_status($uid);
                $Model_withdraw = M('guide_withdraw');
                $Model_amount = M('amount');
                $map_withdraw['uid'] = $map_amount['uid'] = $uid;
                $map_amount['deal_type'] = 1;
                if(in_array($screen,array(0,3))){
                    $temp_withdraw = $Model_withdraw->where($map_withdraw)->field('order_number,withdraw_amount as amount,create_time as time,is_withdraw as result')->select();
                    foreach ($temp_withdraw as $val){
                        $withdraw_list[$val['order_number']] = $val;
                    }
                }
                if(in_array($screen,array(0,1,2))){
                    if($screen == 1)
                        $map_amount['child_id'] = 0;
                    elseif($screen == 2)
                        $map_amount['child_id'] = array('gt',0);
                    $temp_amount = $Model_amount->where($map_amount)->field('order_number,profit_amount as amount,deal_time as time,child_id')->select();
                    foreach ($temp_amount as &$val){
                        $val['order_number'] = M('pay')->getFieldByRequest_pay_no($val['order_number'],'order_number');
                        $amount_list[$val['order_number']] = $val;
                    }
                }
                if(!isset($amount_list))
                    $amount_list = array();
                if (!isset($withdraw_list))
                    $withdraw_list = array();
                $list = array_merge($amount_list,$withdraw_list);
                foreach ($list as &$val){
                    $val['date'] = date('Y-m-d H:i:s',$val['time']);
                    if(isset($val['result']))
                        $val['deal_type'] = 3;
                    elseif (intval($val['child_id']) === 0){
                        $val['deal_type'] = 1;
                        $order_status = M('order')->getFieldByOrder_number($val['order_number'],'server_status');
                        $val['is_cancel'] = $order_status == 2 ? true:false;
                    } else{
                        $val['deal_type'] = 2;
                        $order_info = M('order')->where(array('order_number'=>$val['order_number']))->field('order_money,derate_money')->find();
                        $val['derate_base'] = bcsub($order_info['order_money'],$order_info['derate_money'],2);
                        $val['derate_scale'] = bcmul(bcdiv($val['amount'],$val['derate_base'],2),100,0).'%';
                        $val['child_mobile'] = M('guide_member')->getFieldById($val['child_id'],'mobile');
                    }
                    unset($val['child_id']);
                    $_list[] = $val;
                }
                $list = array_page($_list,$now_page,$page_number,array('time'=>'desc'));
                $this->client_return(1,'获取成功！',$list);
                break;
        }
    }
        ...
        if (IS_GET) {
            ...
            if ($result) {
                ...
                if (!empty($result['idcard'])) {
                    ...
                    $result['idcard'] = Aes_encrypt_decrypt($result['idcard'],2);
                }
            ...
            case 'post':
                ...
                if(false !== $Model->where(array('uid'=>$uid))->save($data)){
                    if($is_getlnglat == 1){//返回对方的经纬度
                        ...
                        $field = 'ui.lon,ui.lat,oi.server_start_time,o.server_status';
                        if($this->client == 1){
                            $join = 'left join __GUIDE_MEMBER_INFO__ as gi on gi.uid = o.guid';
                            $field = 'gi.lon,gi.lat,oi.server_start_time,o.server_status';
                        }
                        ...
                        $lnglat = $Model_order
                            ...
                            ->find();
                        if($lnglat){
                            $lnglat['is_arrive'] = $lnglat['server_status'] == 7 ? 1 : 0;//导游是否已经到达用户附近
                            unset($lnglat['server_status']);
        ...
        switch ($this->_method) {
            case "get":
                ...
                if ($guide_info) {
                    ...
                    $guide_info['idcard']   =   Aes_encrypt_decrypt($guide_info['idcard'],2);
                    //获取用户对导游的评论列表
    ...
    private function withdraw_status($uid){
        ...
        if($list = $Model_withdraw->where($map)->field('id,order_number,create_time,withdraw_amount')->select()){
            $default_time = ini_get('max_execution_time');
            ini_set('max_execution_time', '0');
            foreach ($list as $val){
                $msg = array(
                ...
                switch ($stt){
                    case '0':
                        ...
                        $data_amount = array(
                            ...
                            'deal_type' =>  2,//出账
                            'deal_amount'   =>  $val['withdraw_amount'],
                            'deal_time' =>  NOW_TIME,
                        );
                        break;
                    case '2':
                    ...
                    default:
                        ...
                        //用户提现失败，把钱返回给用户账户
                        $data_guide = array(
                            'now_total_amount'=>array('exp','now_total_amount+'.$val['withdraw_amount']),
                            'action_amount' =>  array('exp','action_amount+'.$val['withdraw_amount']),
                            'out_amount'    =>  array('exp','out_amount-'.$val['withdraw_amount']),
                        );
                ...
                if(isset($data_amount))
                    M('amount')->add($data_amount);
                if(isset($data_guide))
                    M('guide_member_info')->where(array('uid'=>$uid))->save($data_guide);
                continue;
            }
            ini_set('max_execution_time',$default_time);
            return;
     ...
     * 导游详情
     */
    public function guidews_detail(){
        switch ($this->_method){
            case 'get':
                $guidecode = I('guidecode');
                if(empty($guidecode))
                    $this->client_return(0,'导游证号错误！');
                $info = array('guidecode' =>  $guidecode,);
                $path = 'guidews/detail';
