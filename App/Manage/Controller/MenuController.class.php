...
class MenuController extends AdminController {
    ...
    public function index(){
        ...
        $page_size = 15;
        if($pid){
            $this->data = M('Menu')->where("id={$pid}")->field(true)->find();
        }
        ...
        $count = M("Menu")->where($map)->count();
        $list       =   M("Menu")->where($map)->field(true)->order('sort asc,id asc')->page($now_page,$page_size)->select();
        if($list) {
            foreach($list as &$key){
                if($key['pid']){
