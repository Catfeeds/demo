 ...
 * @param string $tmp 数字索引子节点key转化的属性值
 * @return string
 */
function xml_encode($data, $root = 'think', $item = 'item', $attr = '', $id = 'id', $encoding = 'utf-8', $tmp = '') {
    if (is_array($attr)) {
        $_attr = array();
    ...
    $xml .= "<{$root}{$attr}>";
    $xml .= data_to_xml($data, $item, $id, $tmp);
    $xml .= "</{$root}>";
    return $xml;
}
 ...
 * @return string
 */
function data_to_xml($data, $item = 'item', $id = 'id', $tmp = '') {
    $xml = $attr = '';
    foreach ($data as $key => $val) {
        if (is_numeric($key)) {
            $key = empty($tmp) ? $key : $tmp;
            $id && $attr = " {$id}=\"{$key}\"";
            $key = $item;
        }
        $xml .= "<{$key}{$attr}>";
